class Invoice < ApplicationRecord
	...
	friendly_id :number_name, use: [:sequentially_slugged, :scoped], scope: :franchise_id
	
	#Scope for Admin users
	scope :all_ordered, -> {Invoice.includes(:franchise).order("date_entered DESC")}
	scope :all_recent_pending, -> {Invoice.includes(:franchise, :invoice_items).where(paid: 0).order("date_entered DESC").limit(10)}
  scope :all_recent_posted, -> {Invoice.includes(:franchise, :invoice_items).where(paid: 1).order("date_entered DESC").limit(10)}
  ...
  scope :franchise_all, -> (fran_id) {where(franchise_id: fran_id).order("date_entered DESC")}  
	...
	scope :recent_pending, -> (fran_id) {Invoice.includes(:invoice_items).where(franchise_id: fran_id, paid: 0).order("date_entered DESC").limit(5)}
	scope :recent_posted, ->  (fran_id) {Invoice.includes(:invoice_items).where(franchise_id: fran_id, paid: 1).order("date_posted DESC").limit(10)}
