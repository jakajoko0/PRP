...
def self.description_from_code(r_code)
  ...
  result = TransactionCode.select("description").where("code = ?",r_code)
  if result.length > 0 
...
def self.description_from_id(r_id)
  @trans_by_id ||= TransactionCode.select(:id, :code, :description).map{|e| e.attributes.values}.inject({}) {|arr , trans_code| arr[trans_code[0]] = trans_code[2]; arr}
  return @trans_by_id[r_id] if @trans_by_id[r_id]
  result = TransactionCode.select("description").where("id = ?",r_id)
  if result.length > 0 
