class Remittance < ApplicationRecord
  ...
  scope :franchise_all, -> (fran_id) {where(franchise_id: fran_id).order("year DESC, month DESC")}  
	...
	ROYALTY_RATE = 0.09
	COLLECTIONS_ATTRIBUTES = %w(accounting backwork consulting excluded other1 other2 payroll setup tax_preparation)
	CREDIT_ATTRIBUTES = %w(credit1_amount credit2_amount credit3_amount credit4_amount)
	ATTRIBUTES_FOR_ROYALTY = %w(accounting backwork consulting other1 other2 payroll setup tax_preparation)
	COLLECTIONS_DESC = %w(Accounting Backwork Consulting Excluded Other\ 1 Other\ 2 Payroll Tax Preparation) 
	ATTR_DEFAULT_ZEROS = %w(accounting backwork consulting excluded other1 other2 payroll setup tax_preparation calculated_royalty minimum_royalty royalty credit1_amount credit2_amount credit3_amount credit4_amount late_fees payroll_credit_amount)
  ...
  def prior_year_rebate_current
	...
	def self.get_history(franchise_id)
		Remittance.select("year, month").where("franchise_id = ?", franchise_id).order("year DESC , month DESC")
	end
