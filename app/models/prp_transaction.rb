...
class PrpTransaction < ApplicationRecord
  ...
  scope :royalty_payment_for_franchise_at_id, ->(fran_id, trans_id) { where(franchise_id: fran_id, id: -Float::INFINITY..trans_id, transactionable_type: 'Payment').joins("INNER JOIN payments ON prp_transactions.transactionable_id = payments.id").where(payments: {invoice_payment: 0}) }
  def quick_info
