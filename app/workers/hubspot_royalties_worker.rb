class HubspotRoyaltiesWorker
include Sidekiq::Worker
sidekiq_options :retry => true 
# This is the Hubspot Royalty Worker. It's listening for royalty record changes (new or updateds)
# And will call a webhook with the royalty record attributes to be updated in Hubspot

  def perform(change_hash)
    fr_id = change_hash["franchise_id"]
    fr = Franchise.find(fr_id.to_i)
    change_hash["franchise_number"] = fr.franchise
